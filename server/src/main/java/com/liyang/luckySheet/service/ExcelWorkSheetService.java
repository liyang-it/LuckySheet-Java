...
public class ExcelWorkSheetService {
	...
	// 创建一个固定大小为 64的线程池
	private final ThreadPoolExecutor poolExecutor = new ThreadPoolExecutor(64, 64, 60L, TimeUnit.SECONDS, new ArrayBlockingQueue<>(64));
	...
	public ResponseUtil save(@RequestBody Map<String, String> params){
	...
	public void exportExcel(String gridKey, HttpServletResponse response) throws IOException {
		...
		// 当前模块Excel只有一个Sheet内容,不会出现多个,固定取0下标
		...
		for(int c = 1; c <= maxColumn; c++){
			List<String> head0 = Collections.singletonList(record.get(0).getString("value" + c));
			head.add(head0);
		}
		...
		for (int i = 1; i < record.size(); i++) {
			...
			for(int c = 1; c <= maxColumn; c++){
				dataObj.add(record.get(i).getString("value" + c));
			}
			data.add(dataObj);
		}
